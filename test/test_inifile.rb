libpath = File.expand_path '../lib', __dir__
...
class TestIniFile < Test::Unit::TestCase
  ...
  def test_merge_invalid_hash
    ...
    not_a_hash = [['section_one', %w[foo bar], %w[baz buz]]]
  ...
  def test_parse_encoding
    ...
    assert_equal ini_file['www.substancia.com AutoHTTPAgent (ver *)']['Browser'],
                 "Subst\xE2ncia".force_encoding('ISO-8859-1')
  end
  ...
  def test_value_escaping
    ...
    assert_equal %(There is a tab\tcharacter in here somewhere), escaped['tabs']
    assert_equal %(Who uses these anyways?\r), escaped['carriage return']
    assert_equal %(Trust newline!\nAlways there when you need him.\nSplittin' those lines.), escaped['newline']
    assert_equal %(Who'd be silly enough to put\0 a null character in the middle of a string? Stroustrup would not approve!),
                 escaped['null']
    assert_equal %q(This string \t contains \n no \r special \0 characters!), escaped['backslash']
    assert_equal %(Escaping works\tinside quoted strings!), escaped['quoted']
  end
